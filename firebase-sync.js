...
function merge(c={},l={}){const d=new Set([...(c.deletedActivityIds||[]),...(l.deletedActivityIds||[])]),m=new Map();[...(c.activities||[]),...(l.activities||[])].forEach(a=>{if(a&&a.id&&!d.has(a.id))m.set(a.id,a)});const cp=new Map((c.plan||[]).map(x=>[x.id,x])),lp=new Map((l.plan||[]).map(x=>[x.id,x])),ids=new Set([...cp.keys(),...lp.keys()]);const plan=[...ids].map(id=>{const a=cp.get(id)||{},b=lp.get(id)||{};let o=b.actual&&m.has(b.actual)?{...a,...b}:a.actual&&m.has(a.actual)?{...b,...a}:{...a,...b};if(o.actual&&!m.has(o.actual)){delete o.actual;o.status="planned"}return o});return{activities:[...m.values()],deletedActivityIds:[...d],imports:uniq([...(c.imports||[]),...(l.imports||[])]),plan}}
