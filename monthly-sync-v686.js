...
(function(){
  ...
  async function syncCurrentMonth(){
    ...
    const workouts=PLAN.filter(w=>w.week>=cw&&w.week<=Math.min(40,cw+3)&&["bike","run","swim"].includes(w.sport));
    ...
    try{
      ...
      if(status)status.textContent=`Settimane ${cw}-${Math.min(40,cw+3)} sincronizzate: ${events.length} allenamenti. Forza e riposo esclusi.`;
