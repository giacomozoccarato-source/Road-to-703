...
(function(){
  ...
  async function syncSelectedMonth(){
    ...
    if(!month){if(status)status.textContent="Seleziona il mese da sincronizzare.";return}
    ...
    const workouts=PLAN.filter(w=>w.date&&w.date.startsWith(month)&&["bike","run","swim"].includes(w.sport));
    if(!workouts.length){if(status)status.textContent="Nessun allenamento nel mese selezionato.";return}
    if(button)button.disabled=true;if(status)status.textContent=`Validazione ${monthLabel(month)}...`;
    try{
      ...
      if(status)status.textContent=`${monthLabel(month)} sincronizzato: ${events.length} allenamenti. Forza e riposo esclusi.`;
  ...
  function init(){
    ...
    if(!$("#intervalsMonth")){
      const label=document.createElement("label");label.className="fieldLabel";label.innerHTML='Mese da sincronizzare<input id="intervalsMonth" type="month">';
      weekButton.insertAdjacentElement("afterend",label);
      const button=document.createElement("button");button.id="syncIntervalsMonth";button.className="primary wide";button.textContent="Sincronizza mese selezionato";label.insertAdjacentElement("afterend",button);
    }
    const month=$("#intervalsMonth"),months=[...new Set(PLAN.map(w=>w.date&&w.date.slice(0,7)).filter(Boolean))].sort(),today=new Date().toISOString().slice(0,7);
    month.min=months[0]||"";month.max=months.at(-1)||"";month.value=months.includes(today)?today:(months[0]||today);
    $("#syncIntervalsMonth").onclick=syncSelectedMonth;
  }
